...
RELAY_PINS = {
    "R01_PIT":       5,   # Питатель (импульс 500 мс)
    "R02_C1_UP":     6,   # Подъём основного цилиндра
    "R03_C1_DOWN":   13,  # (в этом сценарии не используется, оставлено для совместимости)
    "R04_C2":        19,  # Цилиндр отвёртки (ON=вниз, OFF=вверх)
    "R05_DI4_FREE":  26,  # Free-run: держать ON = крутится; здесь используем импульс 100 мс
    "R06_DI1_POT":   16,  # Режим «по моменту»: держать ON до ОК
    "R07_DI5_TSK0":  20,  # (не используется в этом сценарии)
    "R08":           21,
}

# Датчики (BCM): True=CLOSE (замкнуто на GND через оптрон/сухой контакт)
SENSOR_PINS = {
    "GER_C1_UP":    17,  # верх C1
    "GER_C1_DOWN":  27,  # низ C1 (в этом сценарии не используется)
    "GER_C2_UP":    22,  # верх отвёртки
    "GER_C2_DOWN":  23,  # низ отвёртки
    "IND_SCRW":     12,  # индуктивный: CLOSE=винт прошёл (импульс)
    "DO2_OK":       24,  # от драйвера момента: CLOSE=момент достигнут
    "PED_START":    25,  # педаль: CLOSE=нажата
}
...
FEED_PULSE_MS = 500               # п.9/16/23: импульс подачі
IND_PULSE_WINDOW_MS = 300         # п.10/17/24: окно контроля IND_SCRW
